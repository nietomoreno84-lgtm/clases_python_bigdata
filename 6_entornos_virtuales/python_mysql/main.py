...
# libros_genero = get_book_by_gender('Realismo mágico')
# df = pd.DataFrame(libros_genero)
# print(df)


# sacar un dataframe que muestre libros entre 20 y 40 euros con precio ascendente
...
# libros_precio_10_20 = get_books_by_price(10, 20, 2)
...
# Devolver un dataframe con el titulo y año de publicación de los libros que esten publicados desde 2000 en adelante y que tengo 300 o mas paginas 

def get_books_by_date_page(date,pages):
    try:
        conn = get_connection()
        cursor = conn.cursor(dictionary=True)
...
# result = get_books_by_date_page(2000,300)
# df = pd.DataFrame(result)
# print (df)

# Top 5 libros más vendidos por número de compras.
...
def delete_compra_by_id(id):
    try:
        ...
        conn.commit()
        ## opcional 
        return 'la compra borrado correctamente ' if cursor.rowcount > 0 else 'no se ha podido borrar la compra , intentalo de nuevo'
    except Error as e:
        print(f"Error: {e}")
        return False
    finally:
